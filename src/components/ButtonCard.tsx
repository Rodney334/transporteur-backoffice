    ...
    <span
      ...
      className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 sm:py-3 rounded-lg transition-all shadow-lg border border-white text-xs sm:text-sm ${
        selected === id && !disabled
          ? "bg-[#FD481A] text-white border-[#FD481A] cursor-pointer"
          : disabled
            ? "bg-gray-200 text-gray-400 opacity-80 cursor-not-allowed"
            : "bg-white text-gray-700 hover:border-[#FD481A] cursor-pointer"
      }`}
    >
      <div
        className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full ${
          selected === id && !disabled ? "bg-white/20" : "bg-[#FD481A]"
