...
function ForgotPasswordSuccessContent() {
  ...
  const handleResendEmail = async () => {
    if (!email) {
      toast.error("Aucun email disponible pour renvoyer le lien.");
      return;
    }
    ...
    try {
      ...
      if (resendCount + 1 >= MAX_RESEND_ATTEMPTS) {
        setResendCooldown(COOLDOWN_DURATION * 2);
      } else if (resendCount > 0) {
        setResendCooldown(COOLDOWN_DURATION);
      }

      toast.success("Un nouveau lien de réinitialisation a été envoyé !", {
        autoClose: 5000,
      });
    } catch (error: any) {
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <p className="text-gray-600 text-sm">
              ...
              passe. Vérifiez également vos spams au cas où.
            </p>
          </div>
          ...
          <div className="pt-6">
            <p className="text-gray-500 text-xs">
              Si vous vous souvenez de votre mot de passe, vous pouvez vous
