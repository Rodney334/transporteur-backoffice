...
import { OrderStatus, PaymentMethod } from "@/type/enum";
...
// // components/OrdersManager/OrdersManager.tsx
// "use client";
...
//       if (userRole === "client" && user?._id) {
//         ordersData = await orderService.getUserOrder(user._id);
//       } else {
...
//   // Commandes filtrées et formatées
//   const filteredAndFormattedOrders = useMemo(() => {
//     const filtered = filterOrders(orders, user, activeTab);
//     return filtered.map(formatOrder);
//   }, [orders, user, activeTab, filterOrders, formatOrder]);
...
//       try {
//         setProcessingAction(`accept-${command.id}`);
//         await onAcceptOrder(command.originalData.id);
//         await fetchOrders();
//         setActiveTab("En cours");
//       } catch (err: any) {
//         console.error("Error accepting order:", err);
...
//       try {
//         setProcessingAction(`reject-${command.id}`);
//         await onRejectOrder(command.originalData.id);
//         await fetchOrders();
//       } catch (err: any) {
//         console.error("Error rejecting order:", err);
//         toast.error(err.response?.data?.message || "Erreur lors du rejet");
...
//       }
//     },
//     [onRejectOrder, fetchOrders]
//   );

//   const handleEnd = useCallback(
//     async (
//       command: FormattedDeliveryCard | FormattedCommandCard | FormattedOrder
//     ) => {
//       if (!onEndOrder) return;

//       try {
//         setProcessingAction(`end-${command.id}`);
//         await onEndOrder(command.originalData.id);
//         await fetchOrders();
//         setActiveTab("Terminées");
//       } catch (err: any) {
//         console.error("Error ending order:", err);
...
//   const handleConfirmPrice = useCallback(async () => {
//     if (!selectedCommand || !onValidatePrice) return;
...
//       const price = parseInt(confirmationPrice, 10);
//       await onValidatePrice(selectedCommand.id, price, paymentMethod);
...
//                         }
//                         onEnd={
//                           onEndOrder ? () => handleEnd(command) : undefined
//                         }
//                         onViewDetails={() => handleViewDetails(command)}
//                         isProcessingAccept={isCommandProcessing(
//                           command.id,
