    ...
    (set, get) => ({
      ...
      handleWebSocketMessage: (event: MessageEvent) => {
        try {
          ...
          switch (type) {
            case "ORDER_CREATED":
              get().handleOrderCreated(payload);
              break;
            case "ORDER_UPDATED":
              get().handleOrderUpdated(payload);
              break;
            case "ORDER_STATUS_CHANGED":
              get().handleOrderStatusChanged(payload);
              break;
            case "ORDER_DELETED":
              get().handleOrderDeleted(payload);
              break;
            case "PRICE_NEGOTIATION":
              get().handlePriceNegotiation(payload);
              break;
            default:
